...
func (w *CurrentWeatherData) CurrentByName(location string) error {
	urlStr := fmt.Sprintf(fmt.Sprintf(baseURL, "appid=%s&q=%s&units=%s&lang=%s"), w.Key, url.QueryEscape(location), w.Unit, w.Lang)
	iotlogger.LogHelper.Info("apiurl", urlStr)
	response, err := w.client.Get(urlStr)
	if err != nil {
		return err
	}
