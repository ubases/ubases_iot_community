...
// 实体转pb对象
func SysAuthRule_e2pb(src *SysAuthRuleEntitys) *protosService.SysAuthRule {
	if src == nil {
		return nil
	}
	pbObj := protosService.SysAuthRule{
...
// pb对象转实体
func SysAuthRuleRes_pb2e(src *protosService.SysAuthRule) *SysAuthRuleTreeRes {
	if src == nil {
		return nil
	}
	entitysObj := SysAuthRuleTreeRes{
		SysAuthRuleEntitys: &SysAuthRuleEntitys{
...
// pb对象转实体
func SysAuthRule_pb2e(src *protosService.SysAuthRule) *SysAuthRuleEntitys {
	if src == nil {
		return nil
	}
	entitysObj := SysAuthRuleEntitys{
		Id:              iotutil.ToString(src.Id),
		Pid:             iotutil.ToString(src.Pid),
		Name:            src.Name,
		Title:           src.Title,
...
func SysAuthRule_SetCommonFiled(model *SysAuthRuleEntitys, userId int64, opterType int) {
	if opterType == 1 {
