// Code generated by sgen,2024-06-11 18:03:36. DO NOT EDIT.
// versions: v1.0.0
//  API结构体封装，请自行根据需要删减字段修改

package entitys

import (
	proto "cloud_platform/iot_proto/protos/protosService"
)

//增、删、改及查询返回
type SysRegionServerEntitys struct {
	Id int64  `json:"id,string,omitempty"`
	Sid string `json:"sid,omitempty"`
	LimitedCount int32  `json:"limitedCount,omitempty"`
	BindedCount int32  `json:"bindedCount,omitempty"`
	MqttServer string `json:"mqttServer,omitempty"`
	HttpServer string `json:"httpServer,omitempty"`
	Describe string `json:"describe,omitempty"`
	EnDescribe string `json:"enDescribe,omitempty"`
	Enabled int32  `json:"enabled,omitempty"`
	Lat float64 `json:"lat,omitempty"`
	Lng float64 `json:"lng,omitempty"`
	Country string `json:"country,omitempty"`
	Province string `json:"province,omitempty"`
	City string `json:"city,omitempty"`
	District string `json:"district,omitempty"`
	CreatedBy int64  `json:"createdBy,string,omitempty"`
	AreaPhoneNumber string `json:"areaPhoneNumber,omitempty"`
	WebsocketServer string `json:"websocketServer,omitempty"`
	MqttPort int32 `json:"mqttPort,omitempty"`
}

//新增参数非空检查
func (s *SysRegionServerEntitys) AddCheck() error {
	return nil
}

//修改参数非空检查
func (s *SysRegionServerEntitys) UpdateCheck() error {
	return nil
}

//查询参数必填检查
func (*SysRegionServerQuery) QueryCheck() error {
	return nil
}

//查询条件
type SysRegionServerQuery struct {
	Page      uint64 `json:"page,omitempty"`
	Limit     uint64 `json:"limit,omitempty"`
	Sort      string `json:"sort,omitempty"`
	SortField string `json:"sortField,omitempty"`
	SearchKey string `json:"searchKey,omitempty"`
	Query   *SysRegionServerFilter `json:"query,omitempty"`
}
//SysRegionServerFilter，查询条件，字段请根据需要自行增减
type SysRegionServerFilter struct {
	Id int64  `json:"id,string,omitempty"`
	Sid string `json:"sid,omitempty"`
	LimitedCount int32  `json:"limitedCount,omitempty"`
	BindedCount int32  `json:"bindedCount,omitempty"`
	MqttServer string `json:"mqttServer,omitempty"`
	HttpServer string `json:"httpServer,omitempty"`
	Describe string `json:"describe,omitempty"`
	EnDescribe string `json:"enDescribe,omitempty"`
	Enabled int32  `json:"enabled,omitempty"`
	Lat float64 `json:"lat,omitempty"`
	Lng float64 `json:"lng,omitempty"`
	Country string `json:"country,omitempty"`
	Province string `json:"province,omitempty"`
	City string `json:"city,omitempty"`
	District string `json:"district,omitempty"`
	CreatedBy int64  `json:"createdBy,string,omitempty"`
	AreaPhoneNumber string `json:"areaPhoneNumber,omitempty"`
	WebsocketServer string `json:"websocketServer,omitempty"`
	MqttPort string `json:"mqttPort,omitempty"`
}

//实体转pb对象
func SysRegionServer_e2pb(src *SysRegionServerEntitys) *proto.SysRegionServer {
	if src == nil {
		return nil
	}
	pbObj := proto.SysRegionServer{
		Id:src.Id,
		Sid:src.Sid,
		LimitedCount:src.LimitedCount,
		BindedCount:src.BindedCount,
		MqttServer:src.MqttServer,
		HttpServer:src.HttpServer,
		Describe:src.Describe,
		EnDescribe:src.EnDescribe,
		Enabled:src.Enabled,
		Lat:src.Lat,
		Lng:src.Lng,
		Country:src.Country,
		Province:src.Province,
		City:src.City,
		District:src.District,
		CreatedBy:src.CreatedBy,
		AreaPhoneNumber:src.AreaPhoneNumber,
		WebsocketServer:src.WebsocketServer,
		MqttPort:src.MqttPort,
	}
	return &pbObj
}

//pb对象转实体
func SysRegionServer_pb2e(src *proto.SysRegionServer) *SysRegionServerEntitys {
	if src == nil {
		return nil
	}
	entitysObj := SysRegionServerEntitys{
		Id:              src.Id,
		Sid:             src.Sid,
		LimitedCount:    src.LimitedCount,
		BindedCount:     src.BindedCount,
		MqttServer:      src.MqttServer,
		HttpServer:      src.HttpServer,
		Describe:        src.Describe,
		EnDescribe:      src.EnDescribe,
		Enabled:         src.Enabled,
		Lat:             src.Lat,
		Lng:             src.Lng,
		Country:         src.Country,
		Province:        src.Province,
		City:            src.City,
		District:        src.District,
		CreatedBy:       src.CreatedBy,
		AreaPhoneNumber: src.AreaPhoneNumber,
		WebsocketServer: src.WebsocketServer,
		MqttPort:        src.MqttPort,
	}
	return &entitysObj
}

// 查询条件转换proto
func (s *SysRegionServerQuery) SysRegionServerQuery_e2pb() *proto.SysRegionServer {
	queryObj := &proto.SysRegionServer{}
	if s.Query != nil {
		queryObj.Describe = s.Query.Describe
		queryObj.Enabled = s.Query.Enabled
		queryObj.Sid = s.Query.Sid
	}
	return queryObj
}
