...
import (
	"cloud_platform/iot_app_oem_service/rpc"
...
func InitServiceClient() {
	...
	RegisterGrpcServiceClient(iotconst.IOT_BASIC_SERVICE, func(name string, client client.Client) {
		ClientConfigDictDataServerService = protosService.NewConfigDictDataService(name, client)
		ClientConfigDictTypeServerService = protosService.NewConfigDictTypeService(name, client)
