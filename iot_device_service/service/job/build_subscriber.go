...
func NewBuildSubscriber() (*BuildSubscriber, error) {
	...
	err = suber.CreateOrUpdateConsumer(ctx, iotconst.NATS_STREAM_DEVICE, []string{iotconst.NATS_SUBJECT_DEVICE_JOB}, appName)
	if err != nil {
		cancel()
		return nil, err
	}
...
func MessageHandler(msg jetstream.Msg) {
	...
	if err != nil {
		iotlogger.LogHelper.Errorf("解析构建信息失败,内容[%s],错误:%s", string(msg.Data()), err.Error())
		return
	}
	data := map[string]interface{}{}
	payload, ok := info.Payload.(string)
