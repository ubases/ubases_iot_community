...
func GetHuaweiClient(accessId string, accessKey string, sign map[string]string, other []string) (*HuaweiClient, error) {
	if len(other) < 2 {
...
func (c *HuaweiClient) SendMessage(template string, param map[string]string, targetPhoneNumber ...string) error {
	code, ok := param["code"]
	if !ok {
	...
	body := buildRequestBody(c.sender, phoneNumbers.String(), template, templateParas, "", c.sign["zh"])
