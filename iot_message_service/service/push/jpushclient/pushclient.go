...
const (
	...
	HOST_ALIASES_SSL = "https://device.jpush.cn/v3/aliases"
...
func (this *PushClient) SendGetAliasesRequest(aliasId string) (string, error) {
	url := fmt.Sprintf("%s/%s?platform=android,ios,quickapp&new_format=true", HOST_ALIASES_SSL, aliasId)
	return Get(url).SetBasicAuth(this.AppKey, this.MasterSecret).String()
}
