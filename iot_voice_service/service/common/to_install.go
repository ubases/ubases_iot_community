...
type ITo interface {
	...
	AxyToVoiceConvert(devId string, adjust bool, dataType string, vpmap *protosService.OpmVoiceProductMap, axyVal interface{}) (*VoiceToAxyResult, error)
}
...
func getFirstVoiceVal(m map[string]interface{}) string {
	//如果是别名模式，则需要返回“；”分隔中的第一项
	if v, ok := m["voiceValSynonym"]; ok {
		tempV := iotutil.ToString(v)
		tempArr := strings.Split(tempV, ";")
		return tempArr[0]
	}
	if v, ok := m["voiceVal"]; ok {
		return iotutil.ToString(v)
	}
	return ""
}
