...
// axyVal 音响传入的数据
// 返回值：设备的值、音响的值、错误
func (s *ToFloat) AxyToVoiceConvert(devId string, adjust bool, dataType string, vpmap *protosService.OpmVoiceProductMap, axyVal interface{}) (*VoiceToAxyResult, error) {
	op := FloatToVoice{}
	switch dataType {
	case DT_ENUM:
		return op.float2enum(vpmap, axyVal)
	case DT_BOOL:
		return op.float2enum(vpmap, axyVal)
	case DT_INT, DT_FLOAT, DT_DOUBLE:
		return op.float2float(devId, adjust, vpmap, axyVal)
	}
	return nil, nil
}
...
// float2enum 浮点型转换为枚举
// [{"val":1,"voiceValSynonym":"","vMin":1,"vMax":100,"vStep":1},{"val":4,"voiceValSynonym":"","vMin":201,"vMax":300,"vStep":1}]
func (FloatToVoice) float2enum(vpmap *protosService.OpmVoiceProductMap, axyVal interface{}) (*VoiceToAxyResult, error) {
	var (
...
// float2float 浮点型转语控浮点型
// {"min":1,"max":4320,"step":30,"unit":"min","multiple":null,"vMin":100,"vMax":43200,"vStep":300,"vUnit":"min"}
func (FloatToVoice) float2float(devId string, adjust bool, vpmap *protosService.OpmVoiceProductMap, axyVal interface{}) (*VoiceToAxyResult, error) {
	var (
		res      = &VoiceToAxyResult{}
		voiceMap map[string]interface{}
	)
	...
	res.Data = ConvertToVoiceNumberRange(devId, adjust, vpmap.AttrDpid, voiceMap, axyVal)
	res.VData = axyVal
	return res, nil
}
