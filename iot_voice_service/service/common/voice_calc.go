...
func ConvertToVoiceNumberRange(devId string, adjust bool, dpid int32, params map[string]interface{}, val interface{}) interface{} {
	...
	//数值转换
	vMin, _ := iotutil.ToFloat64Err(params["vMin"])
	vMax, _ := iotutil.ToFloat64Err(params["vMax"])
	aMax, _ := iotutil.ToFloat64Err(params["max"])
	aMin, _ := iotutil.ToFloat64Err(params["min"])

	var value interface{}
	min := GetMaxValue(aMin, vMin)
	max := GetMaxValue(aMax, vMax)
	...
	r := (v - aMin) / (aMax - aMin)
	vV := vMin + math.Round(((vMax-vMin)*r)*10)/10
