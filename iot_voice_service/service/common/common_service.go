...
func GetTmDeviceInfo(ctx context.Context, devId string, roomId int64, voiceNo string) (entitys.TmDevice, error) {
	...
	proCached := ProductCachedData{}
	pro, err := proCached.GetProduct(productKey)
	if err == nil {
		proImg = pro.ImageUrl
	}
	...
	for k, _ := range params {
		for _, productMap := range voiceProduct.FunctionMap {
			...
			if _, ok := devInfo[dpId]; ok {
				...
				if err == nil {
					res, err := sc.AxyToVoiceConvert(devId, adjust, productMap.DataType, productMap, val)
					if err != nil {
						return nil, err
					}
					data[dpId] = res.Data
					dataVoiceCode[productMap.VoiceCode] = data[dpId]
					dataDpid[productMap.AttrDpid] = data[dpId]
				}
			}
			//
			//switch productMap.ValueType {
