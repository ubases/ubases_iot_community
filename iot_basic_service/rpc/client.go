...
// 全局服务客户端变量
var (
//ClientIotDevice protosService.TIotDeviceTriadService
)

//初始化其它服务的客户端
func initServiceClient() {
	//RegisterGrpcServiceClient(iotconst.SVR_DEVICE_MANAGEMENT, func(name string, client client.Client) {
	//	ClientIotDevice = protosService.NewTIotDeviceTriadService(name, client)
	//	//todo 该微服务下的其它接口在这里创建
	//})
	//todo 其它微服务往后面加
}
