...
type CheckOtaFirmwares struct {
	FirmwareType int32                      `json:"firmwareType"`
	HasUpdate    bool                       `json:"hasUpdate"`
	FirmwareList []*CheckOtaVersionResponse `json:"firmwareList"`
}
...
type CheckOtaVersionResponse struct {
	...
	OriVersion      string         `json:"oriVersion"` //原始版本
...
type PayloadParams struct {
	...
	Md5         string `json:"md5"`
	FirmwareKey string `json:"otaKey"`
...
func CheckOtaVersion_Pd2E(src *protosService.CheckOtaVersionResponse, lang string) *CheckOtaVersionResponse {
	otaParams := &CheckOtaVersionResponse{
		...
		OriVersion:      src.UpgradePublish.OriVersion,
		...
		FirmwareKey:     src.OtaPkg.FirmwareKey,
	}
	if lang == "en" {
		otaParams.Remark = src.UpgradePublish.UpgradeDescEn
		otaParams.FirmwareName = src.OtaPkg.FirmwareNameEn
	}
	if src.Code == 200 {
		//todo 转换http
